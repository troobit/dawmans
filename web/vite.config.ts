...
// web/build itself, so the page is same-origin and no proxy exists. The port is
// `dawmans serve`'s own default (cli.DEFAULT_PORT): the guard checks Origin against
// the port it bound, so a proxy pointed elsewhere is 403 rather than merely unrouted.
const engine = process.env.ENGINE_ORIGIN ?? 'http://127.0.0.1:8722';
