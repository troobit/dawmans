...
The two extractors are also exported bare, as `capitalised_runs` and
`numeric_literals`: `dawmans.answer.ground` reads them as the fact-shaped signal
of its own ungrounded rule. Extraction lives here and is shared so the two specs
cannot drift on what counts as a product term; the policy above each of them —
which span is checked, what is discarded, what a miss means — belongs to the
caller.
...
def capitalised_runs(text: str) -> list[str]:
    """Maximal runs of consecutive Capitalised or ALL-CAPS tokens, in order.

    The extraction primitive on its own, over bare text rather than an entry.
    `dawmans.answer.ground` reads it as arm (a) of the ungrounded rule, which is
    what keeps the two specs from drifting on what counts as a product term.

    The entry-scoped rules do not apply and cannot: there is no entry to
    corroborate a sentence-start capital against and no declared device to
    discard. That caller demands two or more tokens in a run anyway, so a bare
    sentence-opening `Click` never satisfies it.
    """
    return [run for _, run in _runs(text, None)]


def numeric_literals(text: str) -> list[str]:
    """Numeric literals with any attached unit, in order — the other half of
    what `dawmans.answer.ground` reuses."""
    return [literal for _, literal in _numerics(text)]


def _runs(span: str, corroborated: frozenset[str] | None) -> list[tuple[int, str]]:
...
def _is_run_material(text: str, initial: bool, corroborated: frozenset[str] | None) -> bool:
    ...
    `corroborated` is `None` when there is no entry to read the rule against —
    the bare `capitalised_runs` primitive — and the discount is then not applied
    rather than applied against an empty set, which would drop every term a
    sentence happens to open with.
    """
    if not text[0].isupper():
        return False
    if corroborated is None or _is_all_caps(text):
        return True
