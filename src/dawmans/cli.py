...
`serve` is `api/answer-engine`'s entry point and lives here too, but its imports are
deferred into the functions that need them. This module is shared with the ingest
commands, and the two sides install different extras: importing starlette or fastembed at
module import time would break `dawmans --help` in an ingest-only environment, exactly as
importing PyMuPDF would break it in a served one.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "serve":
        # `--root` supplies the same two paths the ingest side derives from it, so one
        # repository root configures both halves; the explicit flags still win.
        static_dir = args.static_dir
        if static_dir is None and (args.root / "web" / "build").is_dir():
            static_dir = args.root / "web" / "build"
        run_serve(
            index_dir=args.index_dir if args.index_dir is not None else index_root,
            manuals_root=(
                args.manuals_root if args.manuals_root is not None else args.root / MANUALS_DIR
            ),
            host=args.host,
            port=args.port,
            static_dir=static_dir,
