...
**Generated:** 2026-08-15 · **Specs:** 4 · **Anchored acceptance criteria:** 409 · **ADRs:** 36 per-spec + 12 cross-cutting · **Task ledgers:** 1 of 4
...
- One `tasks.md` ledger, for `data/manual-corpus`, with the `prerequisites.md` naming what no task
  can do for itself. Its first five phases are implemented: the package, the shared records, both
  source stores, PDF extraction and the committed extraction fixtures, the text-conditioning
  stages — furniture marking, glyph repair and English content selection — and the structural
  stages that turn a span model into `Region[]`: the section map and its three paths, TOC
  anchoring, row and table assembly, unit assembly and the `vendor-manual` load path.
