...
`ui/ask-and-source-picker`: Phase 1 (scaffold, contract types and design tokens) and Phase 2 (engine
client and the turn stream) are done, and Phase 3 (scope, sources and history stores) has begun.
...
- **Ledger:** 47 tasks over 9 phases, two work streams; Phase 1 (scaffold, contract types and
  design tokens) and Phase 2 (engine client, SSE reader, block parser and turn reducer) are done —
  13 tasks — delivering `web/` with its `src/lib/engine/` layer, and Phase 3's scope-store tests
  are written.
...
- The first two implemented phases: `web/` scaffolded for `ui/ask-and-source-picker`, with the
  contract types of `CONTRACTS.md` §1–§4e in `web/src/lib/engine/records.ts`, the §11 contrast
  floors asserted from design tokens, and the whole engine layer — the nine-operation client, the
  SSE turn-stream reader, the append-only §4d block parser and the event → Turn reducer with its
  compile-time outcome and event totality — tested and green.
...
- The **task phase is complete** for all four specs, and implementation is under way: the ui
  ledger's Phases 1 and 2 are done, and the other three ledgers are ready to start.
