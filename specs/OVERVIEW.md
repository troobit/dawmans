...
procedure), 6 (providers, credentials and the state seam) and 7 (conversation and the turn
pipeline) are done.
...
- **Ledger:** 45 tasks; phases 1 (package scaffold and the envelope records, tasks 1–3), 2
  ...
  tasks 14–21), 6 (providers, credentials and the state seam, tasks 22–28) and 7
  (conversation and the turn pipeline, tasks 29–32) are done — the first implemented code in
  the repository.
...
- The first implemented code: the `src/dawmans` package scaffold with the ingest/serve dependency
  ...
  counter and follow-up query assembly) and the turn pipeline itself (the retrieval-and-state
  gather, the pre-flight and in-flight gates, the first-token watchdog, supersede-based
  cancellation, incremental event emission and per-turn timings) — with their tests: phases 1–7
  of that ledger, 32 of 45 tasks.
...
- The **task phase is complete** — all four specs carry a ledger — and implementation has begun:
  `api/answer-engine` phase 8 (the local HTTP surface) is the next
