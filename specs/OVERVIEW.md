...
phases 1 (package scaffold and the envelope records), 2 (the corpus view) and 3 (retrieval and
scoping) are done.
...
- **Ledger:** 45 tasks; phases 1 (package scaffold and the envelope records, tasks 1–3), 2
  (the corpus view, tasks 4–5) and 3 (retrieval and scoping, tasks 6–11) are done — the first
...
- The first implemented code: the `src/dawmans` package scaffold with the ingest/serve dependency
  ...
  corpus view (`CorpusView` and its stat-based revision watch), and retrieval and scoping (the
  device scope, the masked hybrid retrievers, RRF fusion, the two-arm relevance threshold and the
  floor-then-cap allocation) with their tests — phases 1–3 of that ledger, 11 of 45 tasks.
...
- The **task phase is complete** — all four specs carry a ledger — and implementation has begun:
  `api/answer-engine` phase 4 (narrowing from triage entries) is the next unstarted work on that
  ledger, with the other three ledgers unstarted.
